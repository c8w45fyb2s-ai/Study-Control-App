...
struct IOSMoreView: View {
    ...
    private var overdueReviewCount: Int {
    ...
    private var shouldShowExamGoalShortcut: Bool {
        hasLearningAssets && store.snapshot.nextExamGoal() == nil
    }

    private var hasPriorityShortcuts: Bool {
        hasModelSetupIssue
            || totalDraftCount > 0
            || overdueReviewCount > 0
            || store.todayTasks.count > 0
            || shouldShowExamGoalShortcut
    }

    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: StudyDesign.Spacing.normal) {
                if hasPriorityShortcuts {
                    IOSMoreSection(title: "待处理", subtitle: "先处理会影响学习流程的事项") {
                        VStack(spacing: StudyDesign.Spacing.tight) {
                            if hasModelSetupIssue {
                                Button {
                            ...
                            if totalDraftCount > 0 {
                                Button {
                                    requestedSection = .drafts
                                } label: {
                                    IOSMoreNavigationCard(
                                        title: "待确认结果",
                                        subtitle: "先审阅 AI 整理结果，再写入学习系统",
                                        icon: AppSection.drafts.icon,
                                        tint: StudyDesign.Colors.secondary,
                                        countText: "\(totalDraftCount)"
                                    )
                                }
                                .buttonStyle(.plain)
                            }
                            ...
                            if overdueReviewCount > 0 {
                                Button {
                                    ...
                                    IOSMoreNavigationCard(
                                        title: "逾期复习",
                                        subtitle: "优先处理已经过期的复习任务",
                                        icon: "exclamationmark.triangle.fill",
                                        tint: StudyDesign.Colors.danger,
                                        countText: "\(overdueReviewCount)"
                                    )
                                }
                                .buttonStyle(.plain)
                            } else if !store.todayTasks.isEmpty {
                                Button {
                                    store.navigateToReviews()
                                } label: {
                                    IOSMoreNavigationCard(
                                        title: "今日复习",
                                        subtitle: "完成 1 项复习即可打卡",
                                        icon: "calendar.badge.clock",
                                        tint: StudyDesign.Colors.primary,
                                        countText: "\(store.todayTasks.count)"
                                    )
                                }
                                .buttonStyle(.plain)
                            }

                            if shouldShowExamGoalShortcut {
                                Button {
                                    requestedSection = .examGoals
                                } label: {
                                    IOSMoreNavigationCard(
                                        title: "考试目标",
                                        subtitle: "补上日期和科目后，计划会更有方向",
                                        icon: AppSection.examGoals.icon,
                ...
                if hasLearningAssetLinks {
                    IOSMoreSection(title: "学习资产", subtitle: learningAssetsSubtitle) {
                        VStack(spacing: StudyDesign.Spacing.tight) {
                            if !store.snapshot.knowledgePoints.isEmpty {
                                NavigationLink {
                                    ...
                                    IOSMoreNavigationCard(
                                        ...
                                        subtitle: "概念和掌握度",
                                        icon: AppSection.knowledge.icon,
                                        tint: StudyDesign.Colors.primary,
                                        countText: "\(store.snapshot.knowledgePoints.count)"
                                    )
                                }
                                .buttonStyle(.plain)
                            }
                            ...
                            if !store.snapshot.mistakes.isEmpty {
                                NavigationLink {
                                    ...
                                    IOSMoreNavigationCard(
                                        ...
                                        subtitle: "错题和薄弱项",
                                        icon: AppSection.mistakes.icon,
                                        tint: StudyDesign.Colors.secondary,
                                        countText: "\(store.snapshot.mistakes.count)"
                                    )
                                }
                                .buttonStyle(.plain)
                            }
                ...
                IOSMoreSection(title: "工具与报告", subtitle: "低频能力集中在一个入口") {
                    NavigationLink {
                        IOSMoreToolsView()
                    } label: {
                        IOSMoreNavigationCard(
                            ...
                            tint: StudyDesign.Colors.primary,
                            countText: loadBalancePlan.overloadedDayCount > 0 ? "\(loadBalancePlan.overloadedDayCount) 天偏满" : nil
                        )
                    }
                    .buttonStyle(.plain)
                }

                IOSMoreSection(title: "计划与记录", subtitle: "管理考试目标和历史任务") {
                    VStack(spacing: StudyDesign.Spacing.tight) {
                        NavigationLink {
                            ExamGoalsView()
                        } label: {
                            IOSMoreNavigationCard(
                                title: AppSection.examGoals.rawValue,
                                subtitle: examGoalsSubtitle,
                                icon: AppSection.examGoals.icon,
                                tint: StudyDesign.Colors.primary,
                                countText: store.snapshot.nextExamGoal() == nil ? nil : "进行中"
                            )
                        }
                        .buttonStyle(.plain)
                        ...
                        NavigationLink {
                            ...
                            IOSMoreNavigationCard(
                                ...
                                subtitle: "已完成的复习",
                                icon: AppSection.pastTasks.icon,
                                tint: StudyDesign.Colors.secondary,
                                countText: completedTaskCount == 0 ? nil : "\(completedTaskCount)"
                            )
                        }
                        .buttonStyle(.plain)
                    }
                ...
                IOSMoreSection(title: "设置", subtitle: "偏好、提醒和数据") {
                    NavigationLink {
                        SettingsView()
                    } label: {
                        IOSMoreNavigationCard(
                            title: AppSection.settings.rawValue,
                            subtitle: "外观、提醒、隐私和高级项",
        ...
        .navigationDestination(isPresented: requestedSectionIsPresented) {
            moreDestinationView(for: requestedSection ?? .drafts)
        }
        .onAppear {
            consumeRequestedSection(store.navigateToSection)
        }
        .onChange(of: store.navigateToSection) { _, target in
            consumeRequestedSection(target)
        }
    }
    ...
    private var hasLearningAssetLinks: Bool {
        !store.snapshot.knowledgePoints.isEmpty || !store.snapshot.mistakes.isEmpty
    }

    private var learningAssetsSubtitle: String {
        let hasKnowledge = !store.snapshot.knowledgePoints.isEmpty
        let hasMistakes = !store.snapshot.mistakes.isEmpty
        if hasKnowledge && hasMistakes { return "知识点和错题" }
        if hasKnowledge { return "知识点" }
        return "错题"
    }

    private var examGoalsSubtitle: String {
        if let goal = store.snapshot.nextExamGoal() {
            return "\(goal.countdownText()) · \(goal.subjectText)"
        }
        return "设置考试日期、科目、每日时间和目标分数"
    }

    private var toolsSummary: String {
        if loadBalancePlan.overloadedDayCount > 0 {
            return "日历、冲刺和报告 · 有任务负荷需要关注"
        }
        return "日历、冲刺、图谱、章节和学习报告"
    }

    private var requestedSectionIsPresented: Binding<Bool> {
        Binding {
    ...
    private func consumeRequestedSection(_ target: String) {
        guard !target.isEmpty else { return }
        if let destination = moreDestination(for: target) {
            requestedSection = destination
        }
        store.navigateToSection = ""
    }

    private func moreDestination(for target: String) -> AppSection? {
        guard let section = AppSection.allCases.first(where: { $0.rawValue == target }) else {
            return nil
        }

        switch section {
        case .drafts, .knowledge, .mistakes, .calendarPlan, .examSprint, .knowledgeGraph, .documentSections, .reportInsights, .entertainmentRules, .pastTasks, .examGoals, .settings:
            return section
        case .importData:
            return .importData
        case .reviews:
    ...
    @ViewBuilder
    private func moreDestinationView(for section: AppSection) -> some View {
        switch section {
        case .dashboard:
            DashboardView()
        case .planningHome:
        ...
        case .reportInsights:
            StudyReportInsightsView()
        case .entertainmentRules:
            EntertainmentRulesPage()
        case .pastTasks:
            PastTasksView()
        case .examGoals:
            ExamGoalsView()
...
private struct IOSMoreToolsView: View {
    @EnvironmentObject private var store: AppStore
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: StudyDesign.Spacing.roomy) {
                IOSMoreSection(title: "规划工具", subtitle: "安排时间和考试节奏") {
                    VStack(spacing: StudyDesign.Spacing.tight) {
                        toolLink(
                            section: .calendarPlan,
                            subtitle: "按日期查看任务、考试和完成记录",
                            countText: "\(StudyCalendarPlanner.makeDays(snapshot: store.snapshot).filter(\.hasWork).count)"
                        )
                        toolLink(
                            section: .examSprint,
                            subtitle: store.snapshot.nextExamGoal() == nil ? "设置考试目标后生成冲刺动作" : "按倒计时调整复习阶段",
                            countText: store.snapshot.nextExamGoal()?.countdownText()
                        )
                    }
                }

                IOSMoreSection(title: "理解与复盘", subtitle: "查看关联、资料结构和学习质量") {
                    VStack(spacing: StudyDesign.Spacing.tight) {
                        toolLink(
                            section: .knowledgeGraph,
                            subtitle: "查看知识点、错题和来源连接",
                            countText: store.snapshot.knowledgePoints.isEmpty ? nil : "\(store.snapshot.knowledgePoints.count)"
                        )
                        toolLink(
                            section: .documentSections,
                            subtitle: "按章节索引资料和长文本片段",
                            countText: store.snapshot.documents.isEmpty ? nil : "\(store.snapshot.documents.count)"
                        )
                        toolLink(
                            section: .reportInsights,
                            subtitle: reportInsightsSubtitle,
                            countText: loadBalancePlan.overloadedDayCount > 0 ? "\(loadBalancePlan.overloadedDayCount)" : nil
                        )
                    }
                }
            }
    ...
    private var reportInsightsSubtitle: String {
        if loadBalancePlan.overloadedDayCount > 0 {
            return "\(loadBalancePlan.overloadedDayCount) 天任务偏满，查看平衡建议"
        }
        return "查看负荷预测、遗忘风险和建议"
    }

    private func toolLink(section: AppSection, subtitle: String, countText: String? = nil) -> some View {
        NavigationLink {
...
private struct IOSMoreSection<Content: View>: View {
    let title: String
    let subtitle: String
...
private struct IOSMorePriorityBanner: View {
    var title: String
    var subtitle: String
    var icon: String
    var tint: Color
    ...
    var body: some View {
        ...
        .overlay(
            RoundedRectangle(cornerRadius: StudyDesign.Radius.medium)
                .stroke(StudyDesign.Colors.accentHairline.opacity(0.32), lineWidth: 1)
        )
...
private struct IOSMoreNavigationCard: View {
    var title: String
    var subtitle: String
    var icon: String
    ...
    private var accessibilitySummary: String {
        if let countText {
            return "\(title)，\(subtitle)，\(countText)"
        }
